...
## Описание
HTTP-слой по плану не является gateway над gRPC, а реализуется как самостоятельный transport. Для auth нужно создать отдельные endpoint-пакеты, зафиксировать request/response DTO, mapping ошибок и тестовый каркас, согласованный с моделями и gRPC-контрактами.
...
## Критерии приемки
- [x] Для всех auth-ручек существуют отдельные пакеты в `internal/api/`.
- [x] В каждом endpoint-пакете есть `handler.go`, `handler_test.go` и `mocks/`.
- [x] DTO и mapping ошибок согласованы с моделями и gRPC auth-контрактами.
- [x] Пакеты можно реализовывать независимо без скрытых общих зависимостей.
- [x] Есть базовые unit-тесты на успешные и ошибочные auth-сценарии.
